...
class CryptoEngine {
  ...
  static Future<AesGcmResult> aesGcmEncrypt(
          String plaintext, String passphrase) =>
      _symmetricEncrypt(
          pkg_cryptography.AesGcm.with256bits(), plaintext, passphrase);

  static Future<String> aesGcmDecrypt(
          AesGcmResult encrypted, String passphrase) =>
      _symmetricDecrypt(
          pkg_cryptography.AesGcm.with256bits(), encrypted, passphrase);

  // --- Symmetric: ChaCha20-Poly1305 ---

  static Future<AesGcmResult> chacha20Encrypt(
          String plaintext, String passphrase) =>
      _symmetricEncrypt(
          pkg_cryptography.Chacha20.poly1305Aead(), plaintext, passphrase);

  static Future<String> chacha20Decrypt(
          AesGcmResult encrypted, String passphrase) =>
      _symmetricDecrypt(
          pkg_cryptography.Chacha20.poly1305Aead(), encrypted, passphrase);

  static Future<AesGcmResult> _symmetricEncrypt(
      pkg_cryptography.Cipher algo, String plaintext, String passphrase) async {
  ...
  static Future<String> _symmetricDecrypt(pkg_cryptography.Cipher algo,
      AesGcmResult encrypted, String passphrase) async {
    ...
    for (final c in text.codeUnits) {
      if (c >= 65 && c <= 90) {
        result.writeCharCode(
            (c - 65 + keyUpper.codeUnitAt(ki % keyUpper.length) - 65) % 26 +
                65);
        ki++;
      } else if (c >= 97 && c <= 122) {
        result.writeCharCode(
            (c - 97 + keyUpper.codeUnitAt(ki % keyUpper.length) - 65) % 26 +
                97);
        ki++;
      } else {
  ...
  static String hashSha256(String input) =>
      pkg_crypto.sha256.convert(utf8.encode(input)).toString();
  static String hashSha512(String input) =>
      pkg_crypto.sha512.convert(utf8.encode(input)).toString();
  static String hashMd5(String input) => _CustomMd5.hash(input);
  static String hashSha1(String input) => _CustomSha1.hash(input);

  static Future<String> hashPbkdf2(String input,
      {String salt = 't2decode'}) async {
  ...
  static Future<String> ed25519Sign(
      String message, String privateKeyB64) async {
    if (message.isEmpty) {
      throw ArgumentError(_errMessage);
    }
  ...
  static Future<bool> ed25519Verify(
    ...
    final algo = pkg_cryptography.Ed25519();
    final publicKey = pkg_cryptography.SimplePublicKey(
        base64.decode(publicKeyB64),
        type: pkg_cryptography.KeyPairType.ed25519);
    final signature = pkg_cryptography.Signature(base64.decode(signatureB64),
        publicKey: publicKey);
  ...
  static Future<String> x25519SharedSecret(
      String myPrivateKeyB64, String theirPublicKeyB64) async {
    ...
    final theirPublicKey = pkg_cryptography.SimplePublicKey(
        base64.decode(theirPublicKeyB64),
        type: pkg_cryptography.KeyPairType.x25519);
    final sharedSecret = await algo.sharedSecretKey(
        keyPair: keyPair, remotePublicKey: theirPublicKey);
...
class AesGcmResult {
  ...
  String toDisplay() => 'Ciphertext: $ciphertext\nNonce: $nonce\nMAC: $mac';
}
...
class _CustomMd5 {
  ...
  static Uint8List _compute(Uint8List message) {
    ...
    padded.setRange(
        padded.length - 8, padded.length, lengthData.buffer.asUint8List());
    ...
    for (int offset = 0; offset < padded.length; offset += 64) {
      final w = List<int>.generate(
          16, (i) => byteData.getUint32(offset + i * 4, Endian.little));
