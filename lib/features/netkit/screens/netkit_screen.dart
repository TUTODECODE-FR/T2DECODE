...
class _SysInfoTabState extends State<_SysInfoTab> {
  ...
  void _addNeofetchLines(List<TermLine> lines) {
    ...
    lines
        .add(const TermLine('  ─────────────────────────────', TermColor.gray));
    ...
    final kernelVersion = osVersion.contains('(')
        ? osVersion.split('(').last.replaceAll(')', '')
        : osVersion;
    lines.add(
        TermLine('  Kernel:  $kernelName $kernelVersion', TermColor.white));
  ...
  Future<void> _addNetworkInterfaceLines(List<TermLine> lines) async {
    try {
      ...
      for (final iface in interfaces) {
        lines.add(TermLine(
            '${interfaces.indexOf(iface) + 1}: ${iface.name}: <UP,BROADCAST,MULTICAST>',
            TermColor.bold));
        for (final addr in iface.addresses) {
          final type = addr.type == InternetAddressType.IPv4 ? 'inet' : 'inet6';
          lines.add(TermLine(
              '    $type ${addr.address}${addr.isLoopback ? ' scope host' : ' scope global'}',
              addr.isLoopback ? TermColor.gray : TermColor.cyan));
        }
      }
    } catch (_) {
      lines.add(const TermLine(
          '    (impossible de lister les interfaces)', TermColor.red));
    }
  }
  ...
  Future<void> _addDnsLines(List<TermLine> lines) async {
    try {
      ...
      if (result.isNotEmpty) {
        lines
            .add(const TermLine('# DNS resolution is working', TermColor.gray));
  ...
  Future<void> _run() async {
    ...
    lines.add(const TermLine('', TermColor.white));
    lines.add(const TermLine(
        '\$ ip addr show 2>/dev/null || ifconfig -a', TermColor.green));
    await _addNetworkInterfaceLines(lines);

    lines.add(const TermLine('', TermColor.white));
    lines.add(
        const TermLine('\$ cat /etc/resolv.conf 2>/dev/null', TermColor.green));
    ...
    final locale = Platform.localeName;
    lines.add(TermLine(
        ' ${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')} up, $cores CPUs, locale $locale',
        TermColor.white));
...
class _PortScannerTabState extends State<_PortScannerTab> {
  ...
  void _printPortResult(
      TerminalEmulatorState term, int port, _PortState state) {
    final service = _knownPorts[port] ?? 'unknown';
    final portStr = port.toString().padRight(5);
    switch (state) {
      case _PortState.open:
        term.addLine(
            TermLine('$portStr/tcp  open     $service', TermColor.green));
        break;
      case _PortState.closed:
        term.addLine(
            TermLine('$portStr/tcp  closed   $service', TermColor.gray));
        break;
      case _PortState.filtered:
        term.addLine(
            TermLine('$portStr/tcp  filtered $service', TermColor.yellow));
        break;
    }
  }
  ...
  Future<void> _scan() async {
    ...
    if (ports.length > maxPorts) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Veuillez limiter le scan à 256 ports maximum.')),
      );
      return;
    }
    ...
    term.addLine(TermLine(
        '\$ nmap -sT $host -p ${_portsCtrl.text.trim()}', TermColor.green));
    term.addLine(const TermLine('', TermColor.white));
    term.addLine(TermLine(
        'Starting T2DECODE Port Scanner at ${_now()}', TermColor.white));
    term.addLine(
        TermLine('Scanning $host (${ports.length} ports)...', TermColor.white));
    term.addLine(const TermLine('', TermColor.white));
    term.addLine(
        const TermLine('PORT       STATE    SERVICE', TermColor.yellow));
    ...
    term.addLine(const TermLine('', TermColor.white));
    term.addLine(TermLine(
        'Scan done: ${ports.length} ports scanned in ${(sw.elapsedMilliseconds / 1000).toStringAsFixed(2)}s',
        TermColor.white));
    term.addLine(TermLine('$openCount open, $closedCount closed/filtered',
        openCount > 0 ? TermColor.green : TermColor.gray));
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                  child: _input(
                      _portsCtrl, 'Ports (22,80 ou 1-1024)', Icons.numbers)),
              const SizedBox(width: 8),
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _scanning
                        ...
                        : TdcColors.accent.withValues(alpha: 0.18),
          ...
          Text('Plages supportées : 22,80,443 ou 1-1024 ou 80,443,8000-8100',
              style: TextStyle(
  ...
  Widget _input(TextEditingController ctrl, String hint, IconData icon) {
    return TextField(
      ...
      style: const TextStyle(
          color: TdcColors.textPrimary, fontSize: 12, fontFamily: 'monospace'),
...
class _DnsTabState extends State<_DnsTab> {
  ...
  Future<void> _lookup() async {
    ...
    lines.add(const TermLine('', TermColor.white));
    lines.add(
        TermLine('; <<>> T2DECODE DiG 1.0 <<>> $domain ANY', TermColor.gray));
    ...
    try {
      ...
      try {
        ipv6 = await InternetAddress.lookup(domain,
      ...
      for (final addr in ipv4) {
        lines.add(TermLine('$domain.     300    IN    A       ${addr.address}',
            TermColor.cyan));
      }
      for (final addr in ipv6) {
        lines.add(TermLine('$domain.     300    IN    AAAA    ${addr.address}',
            TermColor.cyan));
      }

      if (ipv4.isNotEmpty && ipv4.first.host != domain) {
        lines.add(TermLine(
            '$domain.     300    IN    CNAME   ${ipv4.first.host}.',
            TermColor.white));
      }
      ...
      lines
          .add(TermLine(';; Query time (A):    $ipv4Time ms', TermColor.white));
      if (ipv6.isNotEmpty) {
        lines.add(
            TermLine(';; Query time (AAAA): $ipv6Time ms', TermColor.white));
      }
      lines.add(TermLine(';; WHEN: ${_now()}', TermColor.gray));
      lines.add(TermLine(
          ';; MSG SIZE  rcvd: ${ipv4.length + ipv6.length} records',
          TermColor.gray));

      lines.add(const TermLine('', TermColor.white));
      lines.add(TermLine(
          ';; Total: ${ipv4.length} A + ${ipv6.length} AAAA records resolved',
          TermColor.green));
    } catch (e) {
      sw.stop();
      lines.add(const TermLine('', TermColor.white));
  ...
  Future<void> _batchLookup() async {
    ...
    term.addLine(const TermLine(
        '\$ for d in $_googleCom cloudflare.com $_githubCom mozilla.org wikipedia.org; do dig +short \$d; done',
        TermColor.green));
    term.addLine(const TermLine('', TermColor.white));
    term.addLine(const TermLine(
        'DOMAIN                    IPv4              TIME', TermColor.yellow));
    term.addLine(const TermLine(
        '─────────────────────────────────────────────────', TermColor.gray));
    ...
    for (final domain in domains) {
      ...
      final sw = Stopwatch()..start();
      try {
        final addrs = await InternetAddress.lookup(domain,
        ...
        sw.stop();
        term.addLine(TermLine(
            '${domain.padRight(26)}FAILED${''.padRight(12)}${sw.elapsedMilliseconds}ms',
            TermColor.red));
      }
    }
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    prefixIcon:
                        Icon(Icons.dns, color: TdcColors.accent, size: 16),
              ...
              const SizedBox(width: 6),
              _actionBtn(
                  'Batch', TdcColors.warning, _loading ? null : _batchLookup),
            ],
          ),
          const SizedBox(height: 10),
          Expanded(
...
class _DiagnosticTabState extends State<_DiagnosticTab> {
  ...
  Future<void> _diagInterfaces(TerminalEmulatorState term) async {
    term.addLine(
        const TermLine('[1/5] Checking network interfaces...', TermColor.cyan));
    try {
      final interfaces =
      ...
      if (active.isNotEmpty) {
        for (final iface in active) {
          for (final addr in iface.addresses.where((a) => !a.isLoopback)) {
            term.addLine(TermLine(
                '  ✓ ${iface.name}: ${addr.address}', TermColor.green));
          }
        }
      } else {
        term.addLine(const TermLine(
            '  ✗ No active network interfaces found', TermColor.red));
      }
    } catch (e) {
  ...
  Future<int> _diagDns(
      TerminalEmulatorState term, List<String> dnsTargets) async {
    term.addLine(
        const TermLine('[2/5] Testing DNS resolution...', TermColor.cyan));
    int dnsOk = 0;
    for (final target in dnsTargets) {
      ...
      try {
        ...
        dnsOk++;
        term.addLine(TermLine(
            '  ✓ $target resolved in ${sw.elapsedMilliseconds}ms',
            TermColor.green));
      } catch (_) {
        sw.stop();
        term.addLine(TermLine(
            '  ✗ $target FAILED (${sw.elapsedMilliseconds}ms)', TermColor.red));
      }
    }
    return dnsOk;
  }

  Future<void> _diagLatency(TerminalEmulatorState term, List<String> pingHosts,
      List<String> pingNames, List<double> latencies) async {
    term.addLine(const TermLine(
        '[3/5] Measuring latency (TCP connect)...', TermColor.cyan));
    ...
    for (int t = 0; t < pingHosts.length; t++) {
      ...
      if (durations.isNotEmpty) {
        ...
        term.addLine(TermLine(
            '  ✓ ${pingNames[t].padRight(18)} ${avg.toStringAsFixed(1)}ms avg (${durations.length}/3 ok)',
            color));
      } else {
        term.addLine(TermLine(
            '  ✗ ${pingNames[t].padRight(18)} unreachable', TermColor.red));
      }
    }
  }

  Future<int> _diagServices(TerminalEmulatorState term, List<String> svcHosts,
      List<String> svcNames) async {
    term.addLine(
        const TermLine('[4/5] Testing common services...', TermColor.cyan));
    ...
    for (int s = 0; s < svcHosts.length; s++) {
      ...
      if (d != null) {
        svcOk++;
        term.addLine(TermLine(
            '  ✓ ${svcNames[s].padRight(22)} ${d.inMilliseconds}ms',
            TermColor.green));
      } else {
        term.addLine(TermLine(
            '  ✗ ${svcNames[s].padRight(22)} unreachable', TermColor.red));
      }
    }
    return svcOk;
  }
  ...
  void _diagSummary(
      TerminalEmulatorState term,
      int dnsOk,
      int dnsTargetsLength,
      int svcOk,
      int svcHostsLength,
    ...
    term.addLine(TermLine(
        '  DNS Resolution:  $dnsStatus ($dnsOk/$dnsTargetsLength)', dnsColor));
    term.addLine(TermLine(
        '  Avg Latency:     ${avgLatency.toStringAsFixed(1)}ms',
        _getLatencyColor(avgLatency)));
    term.addLine(TermLine('  Services:        $svcOk/$svcHostsLength reachable',
        svcOk == svcHostsLength ? TermColor.green : TermColor.yellow));
    term.addLine(const TermLine('', TermColor.white));

    // Overall score
    final latencyPoints = _getLatencyPoints(avgLatency);
    final score = ((dnsOk / dnsTargetsLength) * 25 +
            (svcOk / svcHostsLength) * 25 +
            (latencies.isNotEmpty ? 25 : 0) +
            latencyPoints)
        .round();
  ...
  Future<void> _runDiagnostic() async {
    ...
    term.addLine(
        const TermLine('\$ netkit-diagnostic --full', TermColor.green));
    term.addLine(const TermLine('', TermColor.white));
    term.addLine(
        TermLine('T2DECODE Network Diagnostic — ${_now()}', TermColor.bold));
    term.addLine(const TermLine(
        '════════════════════════════════════════════════════',
        TermColor.gray));
    term.addLine(const TermLine('', TermColor.white));
    ...
    if (settings.offlineMode || settings.zeroNetworkMode) {
      term.addLine(const TermLine(
          '[Zero-Network / Offline Mode Active]', TermColor.yellow));
      term.addLine(const TermLine(
          '  All external network tests skipped to preserve privacy.',
          TermColor.white));
      term.addLine(const TermLine('', TermColor.white));
      ...
      term.addLine(const TermLine('', TermColor.white));
      term.addLine(const TermLine(
          '  Connectivity:    OFFLINE (Protected)', TermColor.green));
      term.addLine(
          const TermLine('  DNS Resolution:  SKIPPED', TermColor.yellow));
      term.addLine(const TermLine('  Avg Latency:     0ms', TermColor.green));
      term.addLine(
          const TermLine('  Services:        0/3 reachable', TermColor.yellow));
      term.addLine(const TermLine('', TermColor.white));
      term.addLine(
          const TermLine('  Health Score:    100/100 (Safe)', TermColor.green));
      term.addLine(const TermLine('', TermColor.white));
      term.addLine(
          TermLine('Diagnostic complete at ${_now()}', TermColor.gray));
    ...
    final avgLatency = latencies.isNotEmpty
        ...
        : 0.0;
    _diagSummary(term, dnsOk, dnsTargets.length, svcOk, svcHosts.length,
  ...
  Future<void> _runLatencyTest() async {
    ...
    term.addLine(const TermLine(
        '\$ ping -c 10 $_oneOneOneOne  (TCP connect simulation)',
        TermColor.green));
    term.addLine(const TermLine(
        'PING $_oneOneOneOne ($_oneOneOneOne): TCP port 443', TermColor.white));

    final settings = context.read<SettingsProvider>();
    if (settings.offlineMode || settings.zeroNetworkMode) {
      term.addLine(const TermLine('', TermColor.white));
      term.addLine(const TermLine(
          'ping: Option disabled because Offline / Zero-Network Mode is active.',
          TermColor.yellow));
      if (mounted) setState(() => _running = false);
      return;
    }
    ...
    for (int i = 1; i <= 10; i++) {
      ...
      if (d != null) {
        ...
        term.addLine(TermLine(
            'tcp_seq=$i ttl=57 time=${d.inMilliseconds}ms', TermColor.white));
      } else {
    ...
    term.addLine(const TermLine('', TermColor.white));
    term.addLine(const TermLine(
        '--- $_oneOneOneOne ping statistics ---', TermColor.white));
    final loss = ((10 - times.length) / 10 * 100).toStringAsFixed(0);
    term.addLine(TermLine(
        '10 packets transmitted, ${times.length} received, $loss% loss',
        int.parse(loss) == 0 ? TermColor.green : TermColor.yellow));
    ...
    if (times.isNotEmpty) {
      ...
      final avg =
          (times.reduce((a, b) => a + b) / times.length).toStringAsFixed(1);
      term.addLine(
          TermLine('rtt min/avg/max = $min/$avg/$max ms', TermColor.white));
    }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _actionBtn('Diagnostic complet', TdcColors.success,
                  _running ? null : _runDiagnostic),
              const SizedBox(width: 8),
              _actionBtn('Ping test (10x)', TdcColors.info,
                  _running ? null : _runLatencyTest),
              const Spacer(),
              if (_running)
                const Row(children: [
                  SizedBox(
