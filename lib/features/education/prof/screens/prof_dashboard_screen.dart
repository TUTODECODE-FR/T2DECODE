...
class _ProfDashboardScreenState extends State<ProfDashboardScreen> {
  ...
  final TextEditingController _customTopicController =
      TextEditingController(text: 'Sécurité des accès distants SSH & VPN');
  ...
  Widget build(BuildContext context) {
    return Consumer<EducationProvider>(builder: (context, edu, _) {
          ...
          children: [
                  ...
                  children: [
                    Text('MODULE ENSEIGNANT',
                        style: TextStyle(
                            ...
                            letterSpacing: 1.2)),
                    SizedBox(height: 4),
                    Text('T2DECODE Prof (Tableau de bord LMS)',
                        style: TextStyle(
                ...
                Row(
                  children: [
                      ...
                      onPressed: edu.submissions.isEmpty
                          ...
                          : () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text(
                                        'Export Pronote/ENT copié dans le presse-papier au format CSV !')),
                              );
                            },
                    ...
                    ElevatedButton.icon(
                      ...
                      icon: Icon(
                          edu.isServerRunning ? Icons.stop : Icons.play_arrow),
                      label: Text(edu.isServerRunning
                          ? 'Arrêter Serveur LAN'
                          : 'Lancer Serveur LAN Class'),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: edu.isServerRunning
                            ? TdcColors.danger
                            : TdcColors.success,
                      ),
                    ),
            ...
            TdcCard(
              child: Column(
                ...
                children: [
                  const Text(
                      'Générateur Pédagogique (Curriculum BTS SIO / BUT Info)',
                      style: TextStyle(
                          color: TdcColors.textPrimary,
                          fontWeight: FontWeight.bold,
                          fontSize: 16)),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (v) => setState(
                        () => _selectedCurriculum = v ?? _selectedCurriculum),
                  ),
                  const SizedBox(height: 12),
                  TextField(
                      ...
                      onPressed: edu.isGenerating
                          ...
                          : () {
                              edu.generateCourseWithAI(
                                topic:
                                    '[$_selectedCurriculum] ${_customTopicController.text.trim()}',
                                level: 'BTS SIO / BUT Info',
                              );
                            },
                      icon: const Icon(Icons.auto_awesome),
                      label: Text(edu.isGenerating
                          ? 'Génération IA du cours...'
                          : 'Générer le cours & QCM basé sur le référentiel'),
                    ),
                  ),
                ],
              ),
            ...
            const Text('Tableau des copies et suivi Anti-Triche en direct',
                style: TextStyle(
                    color: TdcColors.textPrimary,
                    fontWeight: FontWeight.bold,
                    fontSize: 18)),
            ...
            if (edu.submissions.isEmpty)
              Container(
                ...
                decoration: BoxDecoration(
                    color: TdcColors.surface, borderRadius: TdcRadius.md),
                child: Center(
                  child: Text(
                      'Aucune copie reçue. Lancez le serveur LAN et demandez aux étudiants de soumettre leurs TP.',
                            ...
                            children: [
                              Text(s.studentName,
                                  style: const TextStyle(
                                      color: TdcColors.textPrimary,
                                      fontWeight: FontWeight.bold)),
                              Text('IP: ${s.studentIp} — ${s.courseTitle}',
                                  style: const TextStyle(
                                      color: TdcColors.textSecondary,
                                      fontSize: 12)),
                              if (hasCheatAlert) ...[
                                const SizedBox(height: 4),
                                Text('⚠️ ALERTE TRICHE : ${s.cheatReason}',
                                    style: const TextStyle(
