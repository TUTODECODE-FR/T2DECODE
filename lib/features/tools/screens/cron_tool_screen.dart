...
class _CronToolScreenState extends State<CronToolScreen> {
  ...
  String _getExplanation() {
    String m = _minuteCtrl.text == '*'
        ? 'Chaque minute'
        : 'À la minute ${_minuteCtrl.text}';
    String h =
        _hourCtrl.text == '*' ? 'Chaque heure' : 'À ${_hourCtrl.text} heures';
    String dm = _domCtrl.text == '*' ? '' : 'Le jour ${_domCtrl.text} du mois';
    String mon = _monthCtrl.text == '*' ? '' : 'En ${_monthCtrl.text}';
    String dw =
        _dowCtrl.text == '*' ? '' : 'Le jour de la semaine ${_dowCtrl.text}';
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
              '${_minuteCtrl.text} ${_hourCtrl.text} ${_domCtrl.text} ${_monthCtrl.text} ${_dowCtrl.text}',
              style: const TextStyle(
  ...
  Widget _buildInputGrid() {
    return Column(
      children: [
        ...
        const Text('* = Tout  |  */5 = Toutes les 5  |  1,5 = 1 et 5',
            style: TextStyle(fontSize: 10, color: TdcColors.textMuted)),
      ],
    );
  }
  ...
  Widget _exampleRow(String label, String code) => InkWell(
        onTap: () {
          ...
          _minuteCtrl.text = pts[0];
          _hourCtrl.text = pts[1];
          _domCtrl.text = pts[2];
          _monthCtrl.text = pts[3];
          _dowCtrl.text = pts[4];
          setState(() {});
        },
        child: Padding(
