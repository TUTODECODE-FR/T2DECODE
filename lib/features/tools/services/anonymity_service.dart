...
enum AnonManualTopic { hostname, mac, user, ipv6, mdns, ttl }

/// Guide de commandes manuelles affichable dans l'UI.
class AnonManualCommand {
  final String title;
  final List<String> commands;
  final String note;

  const AnonManualCommand({
    required this.title,
...
class AnonymityService {
  ...
  // ── Guides manuels par appareil ───────────────────────────

  static List<AnonManualCommand> buildManualCommands({
    required AnonManualTopic topic,
    ...
    final iface = interfaceName.trim().isEmpty ? 'eth0' : interfaceName.trim();
    final host = hostname.trim().isEmpty ? generateHostname() : hostname.trim();
    final nextMac = mac.trim().isEmpty ? generateMac() : mac.trim();
    final user = username.trim().isEmpty ? generateUsername() : username.trim();
    final safeTtl = ttl.clamp(1, 255);

    if (Platform.isLinux) {
      return _linuxManual(topic, iface, host, nextMac, user, safeTtl);
    }
    if (Platform.isMacOS) {
      return _macManual(topic, iface, host, nextMac, user, safeTtl);
    }
    if (Platform.isWindows) {
      return _windowsManual(topic, host, nextMac, user, safeTtl);
    }
    return const [
      AnonManualCommand(
        title: 'Plateforme non supportee',
        commands: ['Aucune commande manuelle disponible sur cette plateforme.'],
        note: 'Utiliser Linux, macOS ou Windows pour cet outil.',
      ),
    ];
  }

  static List<AnonManualCommand> _linuxManual(AnonManualTopic topic,
      String iface, String host, String mac, String user, int ttl) {
    switch (topic) {
      case AnonManualTopic.hostname:
        return [
          AnonManualCommand(
            title: 'Changer le hostname (Linux)',
            commands: [
            ...
            note:
                'Certaines distributions demandent une reconnexion ou un reboot.',
          ),
        ];
      case AnonManualTopic.mac:
        return [
          AnonManualCommand(
            title: 'Changer la MAC (Linux)',
            commands: [
            ...
            note:
                'Selon le pilote, la MAC peut etre verrouillee ou restauree au reboot.',
          ),
        ];
      case AnonManualTopic.user:
        return [
          AnonManualCommand(
            title: 'Creer un utilisateur (Linux)',
            commands: [
              'sudo useradd -m -s /bin/bash $user',
              'sudo passwd $user',
              'id $user',
            ],
            note: 'A executer dans un terminal avec droits sudo.',
          ),
        ];
      case AnonManualTopic.ipv6:
        return [
          const AnonManualCommand(
            title: 'Desactiver IPv6 (Linux)',
            commands: [
              'sudo sysctl -w net.ipv6.conf.all.disable_ipv6=1',
              'sudo sysctl -w net.ipv6.conf.default.disable_ipv6=1',
            ],
            note: 'Applique jusqu au reboot si non persiste dans sysctl.conf.',
          ),
        ];
      case AnonManualTopic.mdns:
        return [
          const AnonManualCommand(
            title: 'Desactiver mDNS/Avahi (Linux)',
            commands: [
              'sudo systemctl stop avahi-daemon',
              'sudo systemctl disable avahi-daemon',
              'systemctl status avahi-daemon',
            ],
            note: 'Le service peut etre absent selon la distribution.',
          ),
        ];
      case AnonManualTopic.ttl:
        return [
          AnonManualCommand(
            title: 'Changer TTL (Linux)',
            commands: [
              'sudo sysctl -w net.ipv4.ip_default_ttl=$ttl',
              'sysctl net.ipv4.ip_default_ttl',
            ],
            note: 'Valeur appliquee immediatement pour les nouveaux paquets.',
          ),
        ];
    }
  }

  static List<AnonManualCommand> _macManual(AnonManualTopic topic, String iface,
      String host, String mac, String user, int ttl) {
    switch (topic) {
      case AnonManualTopic.hostname:
        return [
          AnonManualCommand(
            title: 'Changer le hostname (macOS)',
            commands: [
            ...
            note: 'Les trois noms doivent rester coherents sur macOS.',
          ),
        ];
      case AnonManualTopic.mac:
        return [
          AnonManualCommand(
            title: 'Changer la MAC (macOS)',
            commands: [
            ...
            note:
                'Sur Apple Silicon, certaines interfaces Wi-Fi refusent le changement MAC.',
          ),
        ];
      case AnonManualTopic.user:
        return [
          AnonManualCommand(
            title: 'Creer un utilisateur (macOS)',
            commands: [
              'sudo sysadminctl -addUser $user -password "-"',
              'sudo createhomedir -c -u $user',
              'id $user',
            ],
            note: 'Definir ensuite un mot de passe fort pour ce compte.',
          ),
        ];
      case AnonManualTopic.ipv6:
        return [
          const AnonManualCommand(
            title: 'Desactiver IPv6 (macOS)',
            commands: [
            ...
            note:
                'Adapter les noms d interfaces selon la sortie de networksetup.',
          ),
        ];
      case AnonManualTopic.mdns:
        return [
          const AnonManualCommand(
            title: 'Desactiver mDNS (macOS)',
            commands: [
              'sudo launchctl unload -w /System/Library/LaunchDaemons/com.apple.mDNSResponder.plist',
            ],
            note: 'Impacte AirDrop et AirPlay.',
          ),
        ];
      case AnonManualTopic.ttl:
        return [
          AnonManualCommand(
            title: 'Changer TTL (macOS)',
            commands: [
              'sudo sysctl -w net.inet.ip.ttl=$ttl',
              'sysctl net.inet.ip.ttl',
            ],
            note: 'Requis: session admin/sudo.',
          ),
        ];
    }
  }

  static List<AnonManualCommand> _windowsManual(
      AnonManualTopic topic, String host, String mac, String user, int ttl) {
    final macNoColon = mac.replaceAll(':', '');
    switch (topic) {
      case AnonManualTopic.hostname:
        return [
          AnonManualCommand(
            title: 'Changer le hostname (Windows)',
            commands: [
              'powershell -Command "Rename-Computer -NewName \\"$host\\" -Force -PassThru"',
              'shutdown /r /t 0',
            ],
            note: 'Necessite une session administrateur.',
          ),
        ];
      case AnonManualTopic.mac:
        return [
          AnonManualCommand(
            title: 'Changer la MAC (Windows)',
            commands: [
              'powershell -Command "Get-NetAdapter"',
              'powershell -Command "Set-NetAdapterAdvancedProperty -Name \\"Ethernet\\" -RegistryKeyword \\"NetworkAddress\\" -RegistryValue \\"$macNoColon\\""',
              'powershell -Command "Restart-NetAdapter -Name \\"Ethernet\\" -Confirm:\$false"',
            ],
            note: 'Remplacer "Ethernet" par le vrai nom de l interface reseau.',
          ),
        ];
      case AnonManualTopic.user:
        return [
          AnonManualCommand(
            title: 'Creer un utilisateur (Windows)',
            commands: [
              'net user $user /add',
              'net localgroup Users $user /add',
            ],
            note: 'Lancer le terminal en administrateur.',
          ),
        ];
      case AnonManualTopic.ipv6:
        return [
          const AnonManualCommand(
            title: 'Desactiver IPv6 (Windows)',
            commands: [
            ...
            note: 'Peut demander une reconnexion reseau.',
          ),
        ];
      case AnonManualTopic.mdns:
        return [
          const AnonManualCommand(
            title: 'Desactiver Bonjour/mDNS (Windows)',
            commands: [
              'powershell -Command "Set-Service -Name \\"Bonjour Service\\" -StartupType Disabled"',
              'powershell -Command "Stop-Service -Name \\"Bonjour Service\\" -Force"',
            ],
            note: 'Execute seulement si le service Bonjour est installe.',
          ),
        ];
      case AnonManualTopic.ttl:
        return [
          AnonManualCommand(
            title: 'Changer TTL (Windows)',
            commands: [
              'powershell -Command "Set-ItemProperty -Path \\"HKLM:\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters\\" -Name \\"DefaultTTL\\" -Value $ttl -Type DWord"',
            ],
            note: 'Un redemarrage peut etre necessaire.',
          ),
        ];
    }
  }
