...
class ModuleService {
  ...
  /// Loads a full course from an external module file, given its ID.
  Future<Course?> loadFullCourse(String courseId) async {
    try {
      final dir = await getModulesDirectory();
  ...
  Future<String> readLocalModule(String safeFileName) async {
    final dir = await getModulesDirectory();
    final file =
        File('${dir.path}/${safeFileName.split('/').last.split(r'\').last}');
  ...
  String? _validateModuleMap(Map<String, dynamic> m) {
    ...
    if (m['id'] is! String || (m['id'] as String).length > 64)
      return 'ID invalide ou trop long';
    if (m['title'] is! String || (m['title'] as String).length > 100)
      return 'Titre trop long (>100)';
    if (m['description'] is! String ||
        (m['description'] as String).length > 500)
      return 'Description trop longue (>500)';
    ...
    for (var i = 0; i < content.length; i++) {
      ...
      if (text.length > 102400)
        return 'Chapitre $i: texte trop volumineux (>100KB)';
